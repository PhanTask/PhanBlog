...
import jieba
from ctypes import *
from haystack.backends.nlpir import * 
'''
#NLPIR2014 Lib File (NLPIR64, NLPIR32, libNLPIR64.so, libNLPIR32.so),
#Change libFile when you are not using a Win64 environment:
libFile = 'E:/Anaconda3/Lib/site-packages/django_haystack-2.5.dev0-py3.4.egg/haystack/backends/nlpir/NLPIR.dll'
...
def Seg(paragraph):
    ...
    for a in atoms:
        ...
        # print(a.start,a.length)
        ###yield (i, a.sPOS)  #use this line for python 2.x
        start_pos = int(a.start/3)
        stop_pos = int((a.start + a.length)/3)
        yield (i.decode('UTF8'), a.sPOS.decode('UTF8'),start_pos,stop_pos) #use this line for python 3.x
...
class ChineseTokenizer(Tokenizer):
    def __call__(self, text, **kargs):
        words = Seg(text.encode('UTF8'))#使用NLPIR分词库进行分词
        # words = jieba.tokenize(text, mode="search")#使用jieba分词库进行分词  
        token = Token()
        for (w,nouse,start_pos,stop_pos) in words:
...
if __name__ == "__main__":
    ...
    #print(p.decode('UTF8'))
    for t in Seg(p):
        s = '%s\t%s\t%s\t%s\t%s' % (t[0],t[1],t[2],t[3],translatePOS(t[1]))
        print(s)
		'''